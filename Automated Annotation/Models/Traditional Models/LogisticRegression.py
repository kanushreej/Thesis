...
file_path = '/Users/adamzulficar/Documents/year3/Bachelor Project/Thesis/Automated Annotation/Training Data/UK/Brexit_training.csv'
...
# Load the resampled dataset for model processing
data = resampled_data
...
X = np.stack(X.values)
y = data[['pro_brexit', 'anti_brexit', 'neutral', 'irrelevant']]
            ...
            if any_above_threshold:
                for group in stance_groups:
                    ...
                    if relevant_group:
                        ...
                        if prob_dict[max_stance] > 0.5:  # Threshold can be tuned
                            resolved_stances[i][stances.index(max_stance)] = 1
            else:
                max_stance = max(prob_dict, key=prob_dict.get)
                resolved_stances[i][stances.index(max_stance)] = 1
...
def evaluate_model(X, y, stances, n_splits=10):
    ...
    for train_index, test_index in skf.split(X, y['neutral']):  # Using 'neutral' for stratification
...
stances = ['pro_brexit', 'anti_brexit', 'neutral', 'irrelevant']
evaluate_model(X, y, stances)

#resolved_predictions = predict_and_resolve(X, stances)
