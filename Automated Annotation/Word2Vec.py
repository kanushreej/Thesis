...
# Load Data
print("Loading data...")
labelled_data = pd.read_csv('/Users/kanushreejaiswal/Desktop/Resampled_Brexit_Data.csv')
unlabelled_data = pd.read_csv('/Users/kanushreejaiswal/Desktop/Training Data Removed/BrexitUnlabelled_data.csv')
context_data = pd.read_csv('/Users/kanushreejaiswal/Desktop/UK/BrexitOriginal_data.csv') 
...
context_data['text'] = context_data.apply(combine_title_body, axis=1) 
...
labelled_data['context'] = labelled_data.apply(
    lambda x: build_thread(context_data, x['id']) + (x['text'] if pd.notna(x['text']) else '') if x['type'] == 'comment' else (x['text'] if pd.notna(x['text']) else ''), axis=1
)
...
all_stances = ['pro_brexit', 'anti_brexit', 'pro_climateAction', 'anti_climateAction',
